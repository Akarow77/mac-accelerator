...
- (NSString *)inferredRepositoryPath {
  ...
  for (NSInteger index = 0; index < 8; index++) {
    ...
    NSString *script = [location.path stringByAppendingPathComponent:@"run_coreml_server.sh"];
...
- (NSString *)keyPath {
  ...
  return [[support stringByAppendingPathComponent:@"Mac Accelerator"]
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
  ...
  self.window.title = @"Mac Accelerator — Bench-only 0.3.0";
  self.window.minSize = NSMakeSize(700, 440);
  [self.window center];
  NSView *view = self.window.contentView;

  NSTextField *title = [self label:@"Mac Accelerator"
...
- (void)chooseRepository:(id)sender {
  ...
  panel.title = @"Choose the Mac Accelerator project";
...
- (void)startServer:(id)sender {
  ...
  NSArray<NSString *> *required = @[
    [root stringByAppendingPathComponent:@"run_coreml_server.sh"],
    [root stringByAppendingPathComponent:@".coreml-venv/bin/python"],
    [root stringByAppendingPathComponent:@"artifacts/big_driving.mlpackage"],
    [root stringByAppendingPathComponent:@"usb_ncm_supervisor.py"],
    [root stringByAppendingPathComponent:@"models/big_driving_supercombo.onnx"],
    [root stringByAppendingPathComponent:@"models/big_driving_supercombo_metadata.pkl"],
  ];
  ...
  environment[@"MODEL"] = required[2];
  environment[@"ONNX"] = required[4];
  environment[@"METADATA"] = required[5];
  [environment removeObjectForKey:@"PYTHONPATH"];
  [environment removeObjectForKey:@"PYTHONHOME"];
  ...
  self.performanceActivity = [NSProcessInfo.processInfo
    beginActivityWithOptions:(NSActivityUserInitiatedAllowingIdleSystemSleep | NSActivityLatencyCritical)
                      reason:@"Mac Accelerator Core ML inference"];
